...
(function () {
  ...
  function waLink(p) {
    var msg = "Hello Annie Super Store! \uD83D\uDC4B I saw *" + p.name + "* on your website. Is it still available?";
    return "https://wa.me/" + WA + "?text=" + encodeURIComponent(msg);
  }
  function hitRow(p, withStock) {
    var low = p.stock <= 4;
    var meta = p.catName + (withStock ? ' &middot; <em class="' + (low ? "lowtxt" : "oktxt") + '">' + (low ? "Low stock" : "In stock") + ": " + p.stock + "</em>" : "");
  ...
  if (grid) {
    ...
    var stockById = {};
    DB.forEach(function (p) { stockById[p.id] = p.stock; });
    ...
    function applyFilters() {
      ...
      cards.forEach(function (c) {
        ...
        var s = stockById[id] || 0;
        var ok = hitIds[id] &&
          (chipState === "all" || (chipState === "low" ? s <= 4 : s > 4));
        c.hidden = !ok;
        if (ok) shown++;
      });
  ...
  function openModal(p) {
    ...
    var low = p.stock <= 4;
    document.getElementById("pmStock").textContent = (low ? "Low stock" : "In stock") + ": " + p.stock + " available";
    document.getElementById("pmDot").className = "prod__dot" + (low ? " prod__dot--low" : "");
